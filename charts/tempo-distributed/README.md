...
>>>>>>> 8f1fd85 ([tempo-distributed] allow configuration of the kafka receiver)
...
| server.http_listen_port | int | `3100` | HTTP server listen host |
| server.log_format | string | `"logfmt"` | Log format. Can be set to logfmt (default) or json. |
| server.log_level | string | `"info"` | Log level. Can be set to trace, debug, info (default), warn error, fatal, panic |
